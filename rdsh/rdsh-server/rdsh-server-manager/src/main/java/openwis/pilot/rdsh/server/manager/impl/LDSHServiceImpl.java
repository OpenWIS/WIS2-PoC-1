...
public class LDSHServiceImpl implements LDSHService {
	...
	private Ldsh getLDSHById(String ldshId) {
		return new JPAQueryFactory(em).selectFrom(qLdsh)
				.where(qLdsh.id.eq(ldshId)).fetchOne();
	}

	@Override
	public List<LDSHDTO> getLDSH() {
		final List<Ldsh> ldshList = new JPAQueryFactory(em).selectFrom(qLdsh)
				.orderBy(qLdsh.title.asc()).fetch();
	...
	public void deleteLDSH(String ldshId) {
		new JPAQueryFactory(em).delete(qLdsh).where(qLdsh.id.eq(ldshId))
				.execute();
	}
	...
	public boolean validateLDSHToken(String ldshToken) {
		return new JPAQueryFactory(em).selectFrom(qLdsh)
				.where(qLdsh.token.eq(ldshToken)).fetchOne() != null;
	}

	@Override
	public LDSHDTO getLDSHbyToken(String ldshToken) {
		return  new LDSHMapperImpl().toLDSHDTO(new JPAQueryFactory(em).selectFrom(qLdsh)
				.where(qLdsh.token.eq(ldshToken)).fetchOne());
	}
}
