...
private static final Logger logger = Logger.getLogger(LoginWS.class.getName());
	...
	public Response login(openwis.pilot.awisc.server.common.dto.User user) {
	     ...
	     return Response.ok(loginService.login(user))
				  .header("Authorization",LoginHelper.generateToken())
	...
	public Response logout(String logoutMessage) {
		...
		if(LoginHelper.getToken(headers.getRequestHeader("Authorization").get(0)).equals(headers.getRequestHeader("Authorization").get(0)) && (logoutMessage.equals("removeToken")))
		{
			//logger.log(Level.INFO, "DELETE  TOKEN-------------------------------------------> "+LoginHelper.getToken());

			return Response.ok(LoginHelper.deleteToken((headers.getRequestHeader("Authorization").get(0))))
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_METHODS, "DELETE PUT POST")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_CREDENTIALS, "true")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_ORIGIN, "*")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_HEADERS, "origin, content-type, accept, Origin, Content-type, Accept, authorization, Authorization")
	                  .build();
		}
		else {
			return Response.ok("tokenNotRemoved")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_METHODS, "DELETE PUT POST")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_CREDENTIALS, "true")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_ORIGIN, "*")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_HEADERS, "origin, content-type, accept, authorization , Origin, Content-type, Accept, Authorization")
	...
	public Response testService(String user) {
		...
		if(LoginHelper.getToken(headers.getRequestHeader("Authorization").get(0)).equals(headers.getRequestHeader("Authorization").get(0)))
		{
			return Response.ok("I am secure")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_METHODS, "DELETE PUT POST")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_CREDENTIALS, "true")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_ORIGIN, "*")
	                  .header(CorsHeaderConstants.HEADER_AC_ALLOW_HEADERS, "origin, content-type, accept, Origin, Content-type, Accept, authorization, Authorization")
